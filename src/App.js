...
function App() {
  ...
  if (loading) return (<Box pad="150px 0">
                  <Loader color={Theme.PrimaryColor} section />
                </Box>);

  if (error) return <EmptyState type="users" title="Something went wrong please try again." />;
...
const handleFavorite = (character) => {
    ...
    if (selectedCharacters.includes(character.id)) {
        const filterIds = selectedCharacters.filter(item => item !== character.id)
        const filteredCharacters = allCharacters.filter(item => item !== character.id)
  ...
  return (
    ...
    <Layout theme={Theme}>
       <StyledDiv>
        <Header data={data} downloadData={downloadData} selectedCharacters={selectedCharacters} handleFilter={handleFilter}/>

    <Grid default="1fr 1fr 1fr" gap="30px">
    
      {allCharacters.map((elem) => (
              <Card handleFavorite={handleFavorite} selectedCharacters={selectedCharacters} key={elem.id} elem={elem} />
            ))} 
  </Grid>
