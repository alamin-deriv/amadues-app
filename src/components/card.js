import { Box, Text, Tag } from "flexibull-meme";
        ...
        <Box>
          ...
          >
            <Text color="red" size="20px">
              {selectedCharacters.includes(elem.id) ? (
                <i className="form-heart" />
              ) : (
                <i className="form-heart-empty" />
              )}
            </Text>
          </Box>
        </Box>
      ...
      >
        ...
        {elem.films.map((film, index) => (
          <Tag
            solid
            small
            color={stringToHslColor(film, 70, 80)}
            fontColor={stringToHslColor(film, 50, 30)}
            spaceRight
            style={{ marginBottom: "10px" }}
            key={index}
          >
            <Text bold uppercase size="9px" style={{ opacity: 0.8 }}>
              {film}
            </Text>
          </Tag>
        ))}
